...
| `step_timeout_min` | 60 | 单步硬超时（防跑飞）。Goal 模式里这是硬截止；provider `/goal --budget` 只是软计数 |
...
| `grok_build_bin` / `grok_build` | 空 / 关闭 | Grok 原生腿；`max_parallel` 同样默认 24。`model` 为具体 id 时是显式钉；`stable` 表示新 attempt 只读 `grok --no-auto-update models` 的 `Default model`（Grok CLI 1.0.40 为 `grok-4.7`），不读 `~/.grok/config.toml`，也不把 `grok-4.7-build-fast` 当默认。已写入的具体 id 在重试/续跑中保持不变。`migrate-grok-model -config FILE` 只把该文件里遗留的 `grok-4.6` 模型字段改成 `stable`，`-rollback` 恢复迁移前字节；启动时不迁移。派发前 value-blind preflight 持久化认证/代理/限额/模型状态，失败不消耗语义 attempt。Goal 模式：`workflow goal-run -manual` 走交互式 `/goal`（`manual-only`：complete 与同 session 重启已证明；自动与进行中 pause/resume 未验证；`-p` 不是 goal 证明）；`goal-sync` 不启动 provider 但会更新阶段事实。独立设计须用 `-design-receipt` 或已完成设计任务，不改变 `model=fable` |
| `cursor_bin` / `cursor_model` / `cursor_fable` | 空 / 关闭 | 当前 `cursor-agent models` 的 Grok 是 `grok-4.7-xhigh`，同系列还有 `low`/`medium`/`high`，没有 `cursor-` 前缀；Fast 变体分开，不是默认。显式旧钉 `cursor-grok-4.6-xhigh` 仍可执行并显示为 Grok 4.6。`migrate-grok-model` 只把配置里的遗留 `cursor_model` 从该旧默认改成 `grok-4.7-xhigh`，不改 live 配置。显式 Fable 必须 `claude-fable-5-thinking-max`，角色固定 general 且只读。确认 quota 或 eligible 已证明 presemantic failure 后只生成 A=目录稳定 Grok/`xhigh`（显式 `grok-4.6` 或 `stable`）一份答案，随后 B=`gpt-5.6-sol/ultra` 作为唯一自动 Codex 调用，读取原问题/证据与 A，重建目标约束、对抗并修复后直接终局；profile `merge` 必须省略。语义/验收失败不触发；无 blind Sol answer、Sol/max 第三腿或 review-of-review，未决 P0/P1/uncertainty held for Owner |
