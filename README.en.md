...
| `sequence` | Preset prompt sequence: several steps run in order within the same session (chained via `--resume` for continuous context) | acceptEdits; Sonnet→catalog-stable Grok/high (CLI 1.0.40 default grok-4.7), with eligible serial Kimi K3/max continuation; complex frontend adds the risk-appropriate Sol final gate |
...
| `progress-pull` | Progress-pull session: `--resume` a session and have it emit a structured progress report to disk | Read-only; Haiku→catalog-stable Grok/high (CLI 1.0.40 default grok-4.7), with eligible continuation limited to Kimi or the already-proven OpenCode Go lightweight lane |
