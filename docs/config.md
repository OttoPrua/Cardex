...
| `grok_build_bin` / `grok_build` | 空 / 关闭 | Grok 原生腿；`max_parallel` 同样默认 24。派发前 value-blind preflight 持久化认证/代理/限额/模型状态，失败不消耗语义 attempt。Goal 模式：`workflow goal-run -manual` 走交互式 `/goal`（`manual-only`：complete 与同 session 重启已证明；自动与进行中 pause/resume 未验证；`-p` 不是 goal 证明）；`goal-sync` 不启动 provider 但会更新阶段事实。独立 Astra 设计须用 `-design-receipt` 或已完成设计任务，不改变 `model=fable` |
